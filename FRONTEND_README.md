...
- ☁️ **数据同步**：自动上传测速记录到后端服务器（通过 HTTPS）
- 📊 **网络评估**：基于历史数据生成网络质量报告
...
# ⚠️ 注意：因为使用了樱花 FRP 的自动 HTTPS 功能，这里必须用 HTTPS！
API_BASE_URL=https://www-speedtest-i.hcb8c115c.nyat.app:11152/
```

**重要说明**：
- **生产环境**：使用 `https://` 开头的 FRP 域名（如示例）
- **开发环境（模拟器）**：可使用 `http://10.0.2.2:3000/`（仅限本地调试）
- **真机调试**：可使用 `http://192.168.x.x:3000/`（电脑局域网 IP）
- **永远不要混用**：FRP 启用自动 HTTPS 后，必须用 HTTPS 访问
...
4. 在 `AndroidManifest.xml` 中自动注入（Gradle 已配置）

---

## ⚙️ 配置详解

### `local.properties` 参数说明

| 参数 | 说明 | 示例值 | 必需 |
|------|------|--------|------|
| `sdk.dir` | Android SDK 路径（自动生成） | `C:\Users\...\Sdk` | ✅ 自动 |
| `AMAP_WEB_KEY` | 高德 Web 服务 Key（HTML 报告地图） | `0fe0d875...` | ❌ 可选 |
| `AMAP_ANDROID_KEY` | 高德 Android SDK Key | `6565f23f...` | ❌ 可选 |
| `API_BASE_URL` | 后端 API 基础 URL | `https://www-speedtest-i.hcb8c115c.nyat.app:11152/` | ✅ 必需 |
...
└────────┬────────┘
         │
         │ HTTPS Request
         │ GET/POST https://www-speedtest-i.hcb8c115c.nyat.app:11152/api/v1/...
         ↓
┌─────────────────────────────────────────────┐
│   樱花 FRP 服务端（公网）                  │
│  - SSL 终端（自动 HTTPS）                  │
│  - 解密 HTTPS → 内部 HTTP 转发             │
│  - 证书：*.nyat.app Let's Encrypt 通配符  │
└────────────┬───────────────────────────────┘
             │
             │ TCP 隧道（加密）
             ↓
...
| 组件 | 协议 | 地址 | 端口 |
|------|------|------|------|
| Android → FRP | **HTTPS** | `www-speedtest-i.hcb8c115c.nyat.app` | `11152` |
| FRP 客户端 → 后端 | **HTTP** | `127.0.0.1` | `3000` |

**重要**：
- Android 必须使用 `https://`（因为 `*.nyat.app` 强制 HSTS）
- 后端保持 `http://` 监听（FRP 负责 HTTPS 终端）
- **不要在后端配置 SSL 证书**（FRP 已处理）
...
**Token 存储**：`AuthTokenStore` 使用 `SharedPreferences` 持久化保存。
...
- 坐标转换：`CoordTransform`（WGS-84 ↔ GCJ-02 火星坐标）（若为谷歌定位，但高德定位无需）
...
**前端展示**：`EvaluationScreen.kt` 显示分数、评级、建议与历史趋势。
...
| `404` | 接口不存在 | 检查 API_BASE_URL 是否正确 |
| `500` | 服务器内部错误 | 查看后端日志 |
| `-1` | 网络连接失败 | 检查网络与 FRP 隧道状态 |
| `SSLHandshakeException` | HTTPS 证书错误 | 检查 FRP 自动 HTTPS 是否启用 |

### 连接失败排查清单

1. **检查 `local.properties`**
   ```properties
   API_BASE_URL=https://www-speedtest-i.hcb8c115c.nyat.app:11152/
   ```
   - ✅ 协议是 `https://`
...
3. **验证 FRP 穿透**
   ```bash
   curl https://www-speedtest-i.hcb8c115c.nyat.app:11152/api/v1/health/
   # 应返回相同 JSON
   ```

4. **检查 FRP 客户端日志**
   - 应看到：`已从服务器为 www-speedtest-i.hcb8c115c.nyat.app 加载证书`
   - 应看到：`TCP 隧道启动成功`

5. **手机测试**
   - 用手机浏览器访问：`https://www-speedtest-i.hcb8c115c.nyat.app:11152/api/v1/health/`
   - 应看到 JSON（使用移动数据，不要用 WiFi）

6. **Android 网络权限**
   确认 `AndroidManifest.xml` 有：
   ```xml
...
虽然 `*.nyat.app` 证书可信，但可添加网络安全配置以增强控制：
...
<network-security-config>
    <domain-config>
        <domain includeSubdomains="true">www-speedtest-i.hcb8c115c.nyat.app</domain>
        <trust-anchors>
...
5. **手机浏览器测试**
   - 打开手机浏览器
   - 访问：`https://www-speedtest-i.hcb8c115c.nyat.app:11152/api/v1/health/`
   - 应看到 JSON（如看到证书警告，说明证书未生效）

6. **检查 FRP 日志关键词**
   ```
   ✅ 已从服务器为 www-speedtest-i.hcb8c115c.nyat.app 加载证书
   ✅ TCP 隧道启动成功
   ❌ Nyat APP SSL 证书签发中（等待）
   ❌ certificate signed by unknown authority（证书失败）
   ```

---

### 症状：SSLHandshakeException / SSLException
...
**排查**：
1. 查看后端控制台错误堆栈
2. 检查 `data/local-db.json` 文件权限
3. 尝试简单用户名：`test123`，密码：`123456`
4. 确保 `JWT_SECRET` 已设置（不是默认值）
...
**最后更新**: 2025-06-04  
**维护者**: @kevinmou  
**通信架构**: Android (HTTPS) → FRP (SSL Termination) → Backend (HTTP)
